...
local MINOR_VERSION = "$Revision: 15970 $"
...
local function ResetVariables(self)
    ...
    local petSpellCount, petToken = HasPetSpells()
    if (petSpellCount) then
        ...
        local j = spellCount + i
        if (spellNameNew) then
            self.hash[j] = spellName
            self.data[spellName] = spellData
            spellData.IdStart = j
        else
            self.hash[j] = spellData.IdStart
        end
...
local function InitializeEventRegisters(self)
    ...
    self:RegisterEvent("UNIT_PET", "LEARNED_SPELL_IN_TAB")
